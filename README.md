...
- **⏰ Account-Based Time Logging** - "Log 2 hours to team-alpha account for PROJ-123"
...
> - `ACCOUNT_MAPPINGS`: Account to project mappings (e.g., "team-alpha:PROJ,DEV;team-beta:SUPPORT")
...
#### Account-Based Time Logging Tools

- `list_accounts`: List all available accounts for time logging
- `get_account_projects`: Get projects associated with a specific account
- `log_time_with_account`: Log time to an account-specific project
...
## Account-Based Time Logging

### Overview

The account-based time logging feature provides a business-level abstraction over Jira projects, allowing you to:

- **Group Projects by Account**: Organize related projects under logical accounts (e.g., teams, clients, departments)
- **Simplified Time Tracking**: Log time at the account level without needing to know specific project keys
- **Cross-Project Management**: Manage multiple projects within an account as a single unit
- **Business Intelligence**: Track time across logical business units rather than individual projects

### Configuration

Set up account mappings using the `ACCOUNT_MAPPINGS` environment variable:
...
1. **List Available Accounts**:
   ```
   Ask Claude: "List all available accounts for time logging"
   ```

2. **Get Projects for an Account**:
   ```
   Ask Claude: "Show me all projects under the 'team-alpha' account"
   ```

3. **Log Time to an Account**:
   ```
   Ask Claude: "Log 2 hours of development work to team-alpha account for issue PROJ-123"
   ```

### Benefits

- **Simplified Workflow**: Users don't need to remember individual project keys
- **Better Organization**: Projects are grouped by logical business units
- **Consistent Time Tracking**: Standardized approach across different teams and projects
- **Scalability**: Easy to add new projects to existing accounts or create new accounts

For detailed configuration options, see [ACCOUNT_CONFIGURATION.md](ACCOUNT_CONFIGURATION.md).
