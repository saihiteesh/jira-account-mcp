...
# Account-based Time Logging and Project Management Tools


@jira_mcp.tool(tags={"jira", "accounts", "read"})
async def list_accounts(
    ctx: Context,
    search_filter: Annotated[
        str | None,
        Field(
            description="(Optional) Search filter to match account names",
            default=None,
        ),
    ] = None,
) -> str:
    """
    Retrieve all available accounts for time logging and project management.
    
    This tool provides access to account-based abstractions that group projects
    together for better time tracking and project management.
    ...
    Returns:
        JSON string representing a list of account objects with their details.
        Each account includes: id, name, description, project_count, and is_active status.
    ...
    try:
        jira = await get_jira_fetcher(ctx)
        accounts = jira.get_all_accounts(search_filter=search_filter)
        ...
        logger.log(log_level, f"list_accounts failed: {error_message}")
        return json.dumps(error_result, indent=2, ensure_ascii=False)
...
async def get_account_projects(
    ...
    account_id: Annotated[
        str,
        Field(
            description="Account ID to get projects for (e.g., 'default', 'team-alpha')"
        ),
    ],
) -> str:
    """
    Fetch all projects associated with a given account.
    
    This tool retrieves all Jira projects that belong to a specific account,
    allowing for account-based project management and organization.
    
    Args:
        ctx: The FastMCP context.
        account_id: The account ID to retrieve projects for.
        
    Returns:
        JSON string representing a list of project objects under the specified account.
        Each project includes standard Jira project information like key, name, and description.
        
    Raises:
        ValueError: If the Jira client is not configured or the account is not found.
    """
    try:
        jira = await get_jira_fetcher(ctx)
        account_projects = jira.get_account_projects(account_id)
        
        # Get account details for the response
        account = jira.get_account(account_id)
        ...
        logger.log(log_level, f"get_account_projects failed: {error_message}")
        return json.dumps(error_result, indent=2, ensure_ascii=False)


@jira_mcp.tool(tags={"jira", "accounts", "write"})
@check_write_access
async def log_time(
    ctx: Context,
    account_id: Annotated[
        str,
        Field(
            description="Account ID to log time against (required)"
        ),
    ],
    time_spent: Annotated[
        str,
    ...
    description: Annotated[
        str | None,
        Field(description="(Optional) Description of the time logged"),
    ] = None,
    project_id: Annotated[
        str | None,
        Field(description="(Optional) Project ID/key to associate time with"),
    ] = None,
    issue_key: Annotated[
        str | None,
        Field(description="(Optional) Issue key to log time against (e.g., 'PROJ-123')"),
    ] = None,
    started: Annotated[
        str | None,
        Field(
    ...
    """
    Log time against a specific account with optional project and issue association.
    
    This enhanced time logging tool allows logging time at the account level,
    with optional association to specific projects and issues. The account_id
    parameter is required and must be valid.
    
    Args:
        ctx: The FastMCP context.
        account_id: The account ID to log time against (required).
        time_spent: Time spent in Jira format.
        description: Optional description of the time logged.
        project_id: Optional project ID to associate time with.
        issue_key: Optional issue key to log time against.
        started: Optional start time in ISO format.
        
    Returns:
        JSON string with success status, account name confirmation, and time log entry details.
        
    Raises:
        ValueError: If the account is invalid or Jira client is not configured.
    """
    try:
        jira = await get_jira_fetcher(ctx)
        
        # Log time to the account
        result = jira.log_time_to_account(
            account_id=account_id,
            time_spent=time_spent,
            description=description,
        ...
        logger.log(log_level, f"log_time failed: {error_message}")
        return json.dumps(error_result, indent=2, ensure_ascii=False)
